...
namespace geodesuka::core::math {
	...
	real float2x2::operator()(integer RowElement, integer ColumnElement) const {
	...
	real& float2x2::operator()(integer RowElement, integer ColumnElement) {
#ifdef ROW_MAJOR_MEMORY_LAYOUT
		// Row Major Memory Layout
		return this->aptr[RowElement - 1][ColumnElement - 1];
#else
	...
	float2x2 float2x2::operator*(real Rhs) const {
		float2x2 temp;
		for (integer i = 0; i < 2 * 2; i++) {
